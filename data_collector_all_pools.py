...
def extract_features(embeddings):
    ...
    min_features = list(np.min(embeddings, axis=0))
    var_features = list(np.var(embeddings, axis=0))
    return [mean_features, max_features, min_features, var_features]
...
embedds = np.array(embedds).transpose(0, 2, 1)
print(embedds.shape)
...
np.save("train_embeddings_all_pools.npy", train_embedds)
np.save("test_embeddings_all_pools.npy", test_embedds)
np.save("test_labels_all_pools.npy", test_labels)
np.save("train_labels_all_pools.npy", train_labels)
